...
namespace Key {
	...
	void EditorLayer::OnAttach()
	{
		...
		NewScene();
		//OpenScene("assets/scenes/NewSponza.hsc");
	...
	void EditorLayer::UpdateWindowTitle(const std::string& sceneName)
	{
		...
		std::string title = sceneName + " - Keynut - " + Application::GetPlatformName() + " (" + Application::GetConfigurationName() + ") Renderer: " + rendererAPI;
	...
	void EditorLayer::OpenScene()
	{
		...
		std::string filepath = app.OpenFile("Key Scene (*.hsc)\0*.hsc\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	{
		auto& app = Application::Get();
		std::string filepath = app.SaveFile("Key Scene (*.hsc)\0*.hsc\0");
		if (!filepath.empty())
		{
			SceneSerializer serializer(m_EditorScene);
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginPopupModal("About##AboutPopup", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			ImGui::Text("Key Engine");
			ImGui::PopFont();

			ImGui::Separator();
			ImGui::TextWrapped("Key is an early-stage interactive application and rendering engine for Windows.");
			ImGui::Separator();
			ImGui::PushFont(boldFont);
			ImGui::Text("Key Core Team");
			ImGui::PopFont();
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>(KEY_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyPressed(KEY_KEY_LEFT_CONTROL))
		{
			switch (e.GetKeyCode())
			{
				case KeyCode::B:
			...
			if (Input::IsKeyPressed(KEY_KEY_LEFT_SHIFT))
			{
				switch (e.GetKeyCode())
				{
				case KeyCode::S:
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (e.GetMouseButton() == KEY_MOUSE_BUTTON_LEFT && (m_ViewportPanelMouseOver || m_ViewportPanel2MouseOver) && !Input::IsKeyPressed(KeyCode::LeftAlt) && !ImGuizmo::IsOver() && m_SceneState != SceneState::Play)
		{
					...
					for (uint32_t i = 0; i < submeshes.size(); i++)
					{
						...
						if (intersects)
						{
							...
							for (const auto& triangle : triangleCache)
							{
								...
								{
									KEY_WARN("INTERSECTION: {0}, t={1}", submesh.NodeName, t);
