...
class EditorLayer : public Key::Layer
{
public:
	EditorLayer()
	...
	virtual void OnAttach() override
	{
		...
		m_Mesh.reset(new Key::Mesh("assets/models/m1911/m1911.fbx"));
		m_MeshMaterial.reset(new Key::MaterialInstance(m_Mesh->GetMaterial()));

		m_QuadShader = Key::Shader::Create("assets/shaders/quad.glsl");
		m_HDRShader = Key::Shader::Create("assets/shaders/hdr.glsl");

		m_SphereMesh.reset(new Key::Mesh("assets/models/Sphere1m.fbx"));
		m_PlaneMesh.reset(new Key::Mesh("assets/models/Plane1m.obj"));

		m_GridShader = Key::Shader::Create("assets/shaders/Grid.glsl");
		m_GridMaterial = Key::MaterialInstance::Create(Key::Material::Create(m_GridShader));
		m_GridMaterial->Set("u_Scale", m_GridScale);
		m_GridMaterial->Set("u_Res", m_GridSize);

		// Editor
		m_CheckerboardTex.reset(Key::Texture2D::Create("assets/editor/Checkerboard.tga"));

		// Environment
		m_EnvironmentCubeMap.reset(Key::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Radiance.tga"));
		//m_EnvironmentCubeMap.reset(Key::TextureCube::Create("assets/textures/environments/DebugCubeMap.tga"));
		m_EnvironmentIrradiance.reset(Key::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Irradiance.tga"));
		m_BRDFLUT.reset(Key::Texture2D::Create("assets/textures/BRDF_LUT.tga"));

		m_Framebuffer.reset(Key::Framebuffer::Create(1280, 720, Key::FramebufferFormat::RGBA16F));
		m_FinalPresentBuffer.reset(Key::Framebuffer::Create(1280, 720, Key::FramebufferFormat::RGBA8));

		float x = -4.0f;
		float roughness = 0.0f;
		for (int i = 0; i < 8; i++)
		{
			Key::Ref<Key::MaterialInstance> mi(new Key::MaterialInstance(m_SphereMesh->GetMaterial()));
		...
		m_VertexBuffer.reset(Key::VertexBuffer::Create());
		m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

		uint32_t* indices = new uint32_t[6]{ 0, 1, 2, 2, 3, 0, };
		m_IndexBuffer.reset(Key::IndexBuffer::Create());
	...
	virtual void OnUpdate(Key::TimeStep ts) override
	{
		// THINGS TO LOOK AT:
		// - BRDF LUT
	...
	virtual void OnImGuiRender() override
	{
		...
		ImGui::Separator();
		{
			...
			{
				std::string filename = Key::Application::Get().OpenFile("");
				if (filename != "")
				{
					m_Mesh.reset(new Key::Mesh(filename));
					m_MeshMaterial.reset(new Key::MaterialInstance(m_Mesh->GetMaterial()));
				}
			}
		}
			...
			if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Key::Application::Get().OpenFile("");
						if (filename != "")
							m_AlbedoInput.TextureMap.reset(Key::Texture2D::Create(filename, m_AlbedoInput.SRGB));
					}
				}
				ImGui::SameLine();
				ImGui::BeginGroup();
				...
				{
					if (m_AlbedoInput.TextureMap)
						m_AlbedoInput.TextureMap.reset(Key::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
				}
				ImGui::EndGroup();
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Key::Application::Get().OpenFile("");
						if (filename != "")
							m_NormalInput.TextureMap.reset(Key::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Key::Application::Get().OpenFile("");
						if (filename != "")
							m_MetalnessInput.TextureMap.reset(Key::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Key::Application::Get().OpenFile("");
						if (filename != "")
							m_RoughnessInput.TextureMap.reset(Key::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
		...
		{
			auto& shaders = Key::Shader::s_AllShaders;
		...
		auto [wx, wy] = Key::Application::Get().GetWindow().GetWindowPos();
		posX -= wx;
		posY -= wy;
	...
	virtual void OnEvent(Key::Event& event) override
	{
	}
private:
	Key::Ref<Key::Shader> m_QuadShader;
	Key::Ref<Key::Shader> m_HDRShader;
	Key::Ref<Key::Shader> m_GridShader;
	Key::Ref<Key::Mesh> m_Mesh;
	Key::Ref<Key::Mesh> m_SphereMesh, m_PlaneMesh;
	Key::Ref<Key::Texture2D> m_BRDFLUT;

	Key::Ref<Key::MaterialInstance> m_MeshMaterial;
	Key::Ref<Key::MaterialInstance> m_GridMaterial;
	std::vector<Key::Ref<Key::MaterialInstance>> m_MetalSphereMaterialInstances;
	std::vector<Key::Ref<Key::MaterialInstance>> m_DielectricSphereMaterialInstances;
	...
	struct AlbedoInput
	{
		...
		Key::Ref<Key::Texture2D> TextureMap;
		bool SRGB = true;
	...
	std::unique_ptr<Key::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

	Key::Ref<Key::VertexBuffer> m_VertexBuffer;
	Key::Ref<Key::IndexBuffer> m_IndexBuffer;
	Key::Ref<Key::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;
	...
	// Editor resources
	Key::Ref<Key::Texture2D> m_CheckerboardTex;
};
