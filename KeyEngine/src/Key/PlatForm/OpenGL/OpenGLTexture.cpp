...
namespace Key {
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		KEY_RENDER_1(self, {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, KeyToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, KeyToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		KEY_RENDER_S1(srgb, {
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, KeyToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : KeyToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
				glGenerateMipmap(GL_TEXTURE_2D);
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		KEY_RENDER_S({
			glTextureSubImage2D(self->m_RendererID, 0, 0, 0, self->m_Width, self->m_Height, KeyToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
			});
	}
		...
		: m_FilePath(path)
	{
		...
		KEY_RENDER_S3(faces, faceWidth, faceHeight, {
			glGenTextures(1, &self->m_RendererID);
			...
			auto format = KeyToOpenGLTextureFormat(self->m_Format);
