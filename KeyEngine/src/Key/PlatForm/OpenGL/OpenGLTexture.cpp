...
namespace Key {
	...
	static GLenum KeyToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
		case Key::TextureFormat::RGB:     return GL_RGB;
		case Key::TextureFormat::RGBA:    return GL_RGBA;
		case Key::TextureFormat::Float16: return GL_RGBA16F;
		}
		KEY_CORE_ASSERT(false, "Unknown texture format!");
		return 0;
	}
		...
		Renderer::Submit([this]()
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, KeyToOpenGLTextureFormat(m_Format), m_Width, m_Height, 0, KeyToOpenGLTextureFormat(m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		else
		{
			KEY_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			KEY_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([=]()
			{
				...
				if (srgb)
				{
					...
					KEY_CORE_INFO("Creating srgb texture width {0} mips", levels);
				...
				else
				{
					...
					GLenum internalFormat = KeyToOpenGLTextureFormat(m_Format);
					GLenum format = srgb ? GL_SRGB8 : (m_IsHDR ? GL_RGB : KeyToOpenGLTextureFormat(m_Format)); // HDR = GL_RGB for now
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		Renderer::Submit([this]() {
			glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, KeyToOpenGLTextureFormat(m_Format), GL_UNSIGNED_BYTE, m_ImageData.Data);
			});
	}

	void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
	{
		KEY_CORE_ASSERT(m_Locked, "Texture must be locked!");
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([=]() {
			...
			glTextureStorage2D(m_RendererID, levels, KeyToOpenGLTextureFormat(m_Format), width, height);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		: m_FilePath(path)
	{
		...
		KEY_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		Renderer::Submit([=]() {
			...
			auto format = KeyToOpenGLTextureFormat(m_Format);
