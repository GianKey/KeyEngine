...
namespace Key {
	...
	namespace Utils
	{
		...
		static GLenum DataType(GLenum format)
		{
			...
			KEY_CORE_ASSERT(false, "Unknown format!");
			return 0;
		}
		...
		Renderer::Submit([instance]() mutable
			{
				...
				{
					KEY_CORE_ASSERT(instance->m_ColorAttachments.size() <= 4);
				...
				if (multisample)
				{
					...
					// TODO: Create Key texture object based on format here
				...
				KEY_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
