...
namespace Key {
		...
		: m_Path(path), m_Properties(properties)
	{
		...
		KEY_CORE_ASSERT(m_ImageData.Data, "Failed to load image!");
		if (!m_ImageData.Data)
			return;
		...
		KEY_CORE_ASSERT(m_Format != ImageFormat::None);
		...
		: m_Format(format), m_Properties(properties)
	{
		...
		KEY_CORE_ASSERT(format == ImageFormat::RGBA);
	...
	void VulkanTexture2D::Invalidate()
	{
		...
		KEY_CORE_ASSERT(m_ImageData.Data);
