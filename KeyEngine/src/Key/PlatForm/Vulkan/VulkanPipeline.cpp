...
namespace Key {
	...
	void VulkanPipeline::Invalidate()
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			KEY_CORE_ASSERT(instance->m_Specification.Shader);
			...
			if (!shaderDescriptorSets.empty())
			{
				...
				KEY_CORE_WARN("VulkanPipeline - Updating {0} descriptor sets", writeDescriptors.size());
	...
	void VulkanPipeline::RT_SetUniformBuffer(Ref<UniformBuffer> uniformBuffer, uint32_t binding, uint32_t set)
	{
		...
		KEY_CORE_ASSERT(m_DescriptorSets.DescriptorSets.size() > set);
		...
		KEY_CORE_WARN("VulkanPipeline - Updating descriptor set (VulkanPipeline::SetUniformBuffer)");
		VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
