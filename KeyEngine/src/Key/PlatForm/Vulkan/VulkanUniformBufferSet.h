...
namespace Key {
	...
	public:
		...
		{
			KEY_CORE_ASSERT(m_UniformBuffers.find(frame) != m_UniformBuffers.end());
			KEY_CORE_ASSERT(m_UniformBuffers.at(frame).find(set) != m_UniformBuffers.at(frame).end());
			KEY_CORE_ASSERT(m_UniformBuffers.at(frame).at(set).find(binding) != m_UniformBuffers.at(frame).at(set).end());
