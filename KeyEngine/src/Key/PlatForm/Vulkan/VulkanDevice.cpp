...
namespace Key {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			KEY_CORE_TRACE("Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		KEY_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		KEY_CORE_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				KEY_CORE_TRACE("Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					KEY_CORE_TRACE("  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		KEY_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		KEY_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
	...
	void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		KEY_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
