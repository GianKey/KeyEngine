...
namespace Key {
	...
	public:
		...
		UniformBuffer& GetUniformBuffer(uint32_t binding = 0, uint32_t set = 0) { KEY_CORE_ASSERT(m_ShaderDescriptorSets.at(set).UniformBuffers.size() > binding); return *m_ShaderDescriptorSets.at(set).UniformBuffers.at(binding); }
