...
namespace Key {
	...
	void VulkanSwapChain::InitSurface(GLFWwindow* windowHandle)
	{
		...
		KEY_CORE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		KEY_CORE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync)
	{
		...
		KEY_CORE_ASSERT(presentModeCount > 0, "");
