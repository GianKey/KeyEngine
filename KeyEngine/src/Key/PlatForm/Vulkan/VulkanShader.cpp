...
namespace Key {
	...
	void VulkanShader::Reflect(VkShaderStageFlagBits shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			//KEY_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end());
			...
			else
			{
				...
				{
					KEY_CORE_TRACE("Resizing uniform buffer (binding = {0}, set = {1}) to {2} bytes", binding, descriptorSet, size);
					uniformBuffer->Size = size;
					AllocateUniformBuffer(*uniformBuffer);
				}
	...
	{
		KEY_CORE_ASSERT(m_ShaderDescriptorSets.find(set) != m_ShaderDescriptorSets.end());
	...
	{
		KEY_CORE_ASSERT(s_UniformBuffers.find(set) != s_UniformBuffers.end());
		KEY_CORE_ASSERT(s_UniformBuffers.at(set).find(bindingPoint) != s_UniformBuffers.at(set).end());
		KEY_CORE_ASSERT(s_UniformBuffers.at(set).at(bindingPoint));
