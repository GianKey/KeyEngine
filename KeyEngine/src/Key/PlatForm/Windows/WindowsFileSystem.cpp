...
namespace Key {
	...
	bool FileSystem::CreateFolder(const std::string& filepath)
	{
		...
		if (!created)
		{
			...
			if (error == ERROR_ALREADY_EXISTS)
				KEY_CORE_ERROR("{0} already exists!", filepath);

			if (error == ERROR_PATH_NOT_FOUND)
				KEY_CORE_ERROR("{0}: One or more directories don't exist.", filepath);
	...
	unsigned long FileSystem::Watch(void* param)
	{
		...
		if (handle == INVALID_HANDLE_VALUE)
			KEY_CORE_ERROR("Unable to accquire directory handle: {0}", GetLastError());

		overlapped.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);

		if (overlapped.hEvent == NULL)
		{
			KEY_CORE_ERROR("CreateEvent failed!");
			return 0;
		}
