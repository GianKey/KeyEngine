...
namespace Key {
	...
	static void LoadKeyRuntimeAssembly(const std::string& path)
	{
		if (s_AppAssembly)
		{
	...
	{
		KEY_CORE_ASSERT(s_EntityInstanceMap.find(entityID) != s_EntityInstanceMap.end(), "Could not find entity in instance map!");
	...
	void ScriptEngine::OnInitEntity(ScriptComponent& script, uint32_t entityID, uint32_t sceneID)
	{
		...
		// Retrieve public fields
		{
			...
			while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
			{
				...
				auto& publicField = s_PublicFields[script.ModuleName].emplace_back(name, KeyFieldType);
	...
	const Key::ScriptModuleFieldMap& ScriptEngine::GetFieldMap()
	{
