...
namespace Key {
	...
	struct EntityScriptClass
	{
		...
		void InitClassMethods(MonoImage* image)
		{
			...
			OnCollision2DBeginMethod = GetMethod(s_CoreAssemblyImage, "Key.Entity:OnCollision2DBegin(single)");
			OnCollision2DEndMethod = GetMethod(s_CoreAssemblyImage, "Key.Entity:OnCollision2DEnd(single)");
		}
	};

	MonoObject* EntityInstance::GetInstance()
	{
		KEY_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	void ScriptEngine::LoadKeyRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain("Key Runtime", nullptr);
			mono_domain_set(domain, false);

			cleanup = true;
		}

		s_CoreAssembly = LoadAssembly("assets/scripts/Key-ScriptCore.dll");
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadKeyRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		KEY_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		KEY_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					KEY_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					KEY_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		KEY_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		KEY_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	static FieldType GetKeyFieldType(MonoType* monoType)
	{
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			KEY_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	{
		KEY_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		KEY_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
