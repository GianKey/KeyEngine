...
namespace Key {
	...
	{
		KEY_SCOPE_TIMER("SceneRenderer::Init");
		...
		// Geometry
		{
			...
			pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("KeyPBR_Static");
	...
	{
		KEY_CORE_ASSERT(!m_Active, "Can't change scenes while rendering");
		m_Scene = scene;
	}
