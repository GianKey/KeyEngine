...
namespace Key {
	...
	void SceneRenderer::Init()
	{
		...
		geoRenderPassSpec.TargetFramebuffer = Key::Framebuffer::Create(geoFramebufferSpec);
		...
		compRenderPassSpec.TargetFramebuffer = Key::Framebuffer::Create(compFramebufferSpec);
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> SceneRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		KEY_CORE_ASSERT(envEquirect->GetFormat() == TextureFormat::Float16, "Texture is not HDR!");
