...
namespace Key {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			KEY_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		if (!scene || !scene->HasMeshes())
			KEY_CORE_ERROR("Failed to load mesh file: {0}", filename);

		//double factor;
		//scene->mMetaData->Get("UnitScaleFactor", factor);
		//KEY_CORE_INFO("FBX Scene Scale: {0}", factor);

		m_IsAnimated = scene->mAnimations != nullptr;
		m_MeshShader = m_IsAnimated ? Renderer::GetShaderLibrary()->Get("KeyPBR_Anim") : Renderer::GetShaderLibrary()->Get("KeyPBR_Static");
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			KEY_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				KEY_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						KEY_CORE_TRACE("Found existing bone in map");
		...
		if (scene->HasMaterials())
		{
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				KEY_CORE_INFO("Material Name = {0}; Index = {1}", aiMaterialName.data, i);
				aiString aiTexPath;
				uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
				KEY_CORE_TRACE("  TextureCount = {0}", textureCount);

				aiColor3D aiColor;
				aiMaterial->Get(AI_MATKEY_COLOR_DIFFUSE, aiColor);
				KEY_CORE_TRACE("COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				...
				if (aiMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						...
						KEY_CORE_TRACE("  Texture Path = {0}", texturePath);
				...
				else
				{
					...
					KEY_CORE_TRACE("Mesh has no albedo map");
				}
				...
				if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						KEY_CORE_TRACE("  Normal map path = {0}", texturePath);
				...
				{
					KEY_CORE_TRACE("Mesh has no normal map");
				}
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						KEY_CORE_TRACE("  Roughness map path = {0}", texturePath);
				...
				{
					KEY_CORE_TRACE("Mesh has no roughness texture");
				}
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						KEY_CORE_TRACE("  Metalness map path = {0}", texturePath);
				...
				{
					KEY_CORE_TRACE("Mesh has no metalness texture");
				}
				...
				for (uint32_t i = 0; i < aiMaterial->mNumProperties; i++)
				{
					...
					KEY_CORE_TRACE("Material Property:");
					KEY_CORE_TRACE("  Name = {0}", prop->mKey.data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						KEY_CORE_TRACE("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						KEY_CORE_TRACE("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						KEY_CORE_TRACE("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						KEY_CORE_TRACE("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						KEY_CORE_TRACE("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						KEY_CORE_TRACE("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						KEY_CORE_TRACE("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						KEY_CORE_TRACE("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						KEY_CORE_TRACE("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						KEY_CORE_TRACE("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						KEY_CORE_TRACE("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						KEY_CORE_TRACE("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						KEY_CORE_TRACE("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// KEY_CORE_TRACE("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		KEY_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		KEY_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		KEY_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		KEY_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		KEY_CORE_TRACE("------------------------------------------------------");
		KEY_CORE_TRACE("Vertex Buffer Dump");
		KEY_CORE_TRACE("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				KEY_CORE_TRACE("Vertex: {0}", i);
				KEY_CORE_TRACE("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				KEY_CORE_TRACE("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				KEY_CORE_TRACE("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				KEY_CORE_TRACE("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				KEY_CORE_TRACE("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				KEY_CORE_TRACE("--");
			}
		}
		else
