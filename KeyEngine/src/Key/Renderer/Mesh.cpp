...
#if MESH_DEBUG_LOG
#define KEY_MESH_LOG(...) KEY_CORE_TRACE(__VA_ARGS__)
#else
#define KEY_MESH_LOG(...)
#endif
		...
		: m_FilePath(filename)
	{
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						KEY_MESH_LOG("Found existing bone in map");
		...
		{
			KEY_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				KEY_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				KEY_MESH_LOG("    ROUGHNESS = {0}", roughness);
				KEY_MESH_LOG("    METALNESS = {0}", metalness);
				...
				if (hasNormalMap)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					KEY_MESH_LOG("    Normal map path = {0}", texturePath);
				...
				if (hasRoughnessMap)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					KEY_MESH_LOG("    Roughness map path = {0}", texturePath);
					auto texture = Texture2D::Create(texturePath);
					if (texture->Loaded())
					{
						m_Textures.push_back(texture);
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						KEY_MESH_LOG("    Metalness map path = {0}", texturePath);
				...
				{
					KEY_MESH_LOG("    No metalness texture");
				...
				for (uint32_t p = 0; p < aiMaterial->mNumProperties; p++)
				{
					...
					KEY_MESH_LOG("Material Property:");
					KEY_MESH_LOG("  Name = {0}", prop->mKey.data);
					// KEY_MESH_LOG("  Type = {0}", prop->mType);
					// KEY_MESH_LOG("  Size = {0}", prop->mDataLength);
					float data = *(float*)prop->mData;
					KEY_MESH_LOG("  Value = {0}", data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						KEY_MESH_LOG("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						KEY_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						KEY_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						KEY_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						KEY_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						KEY_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						KEY_MESH_LOG("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						KEY_MESH_LOG("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						KEY_MESH_LOG("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						KEY_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						KEY_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						KEY_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						KEY_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
#endif
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		KEY_MESH_LOG("------------------------------------------------------");
		KEY_MESH_LOG("Vertex Buffer Dump");
		KEY_MESH_LOG("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				KEY_MESH_LOG("Vertex: {0}", i);
				KEY_MESH_LOG("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				KEY_MESH_LOG("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				KEY_MESH_LOG("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				KEY_MESH_LOG("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				KEY_MESH_LOG("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				KEY_MESH_LOG("--");
			}
		}
		else
